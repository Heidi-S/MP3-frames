...
function expectParseErrorCode(
  ...
  expect(thrown, `expected ${code} but parsing succeeded`).toBeInstanceOf(
    Mp3ParseError,
  );
...
describe("input-level rejection", () => {
  ...
  it("rejects a file containing no MPEG audio at all", () => {
    expectParseErrorCode(
      Buffer.from("this is a plain text file, not audio", "utf8"),
      "INVALID_SYNC",
    );
  });
  ...
  it("rejects an ID3v2 tag with a non-syncsafe size field", () => {
    const data = Buffer.concat([
      buildId3v2Tag({ malformedSize: true }),
      buildFrame(),
    ]);
...
describe("header-level rejection", () => {
  ...
  it("rejects the reserved bitrate index", () => {
    expectParseErrorCode(
      buildFrame({ bitrateIndex: 0b1111 }),
      "RESERVED_BITRATE",
    );
  });
...
describe("stream-level rejection", () => {
  ...
  it("rejects trailing bytes that are not audio or recognised metadata", () => {
    const data = Buffer.concat([
      buildFrames(2),
      Buffer.from("trailing junk data", "utf8"),
    ]);
