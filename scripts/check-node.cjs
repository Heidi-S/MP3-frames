...
if (isNaN(major) || major < REQUIRED_MAJOR) {
  console.error("");
  console.error(
    "  This project requires Node.js " + REQUIRED_MAJOR + " or newer.",
  );
  console.error("  You are running Node.js " + current + ".");
  console.error("");
  console.error(
    "  The dev/test tooling (tsx, vitest) requires Node 18+, and the",
  );
  console.error(
    "  source uses modern JavaScript that older versions cannot parse.",
  );
  console.error("");
